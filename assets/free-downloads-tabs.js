...
if (tabSections.length) {
  ...
  const toggleTab = () => {
    const tabButtonsOnDesktop = document.querySelectorAll(
      ".free-downloads-tabs__buttons .tab-trigger"
    );
    const tabButtonsOnMobile = document.querySelectorAll(
      ".section-tab .tab-trigger"
    );
    ...
    if (ol.length) {
      ol.forEach((list) => {
        ...
        if (firstLi && secondLi) {
          ...
          console.error("One or both list items not found.");
        }
      });
    }
  };
